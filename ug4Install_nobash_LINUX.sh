...
mkdir build &&
cd build &&
cmake -DSTATIC_BUILD=ON -DEMBEDDED_PLUGINS=ON -DCMAKE_BUILD_TYPE=vrl -DLAPACK=OFF -DBLAS=OFF -DPARALLEL=OFF -DConvectionDiffusion=ON -DElectromagnetism=ON -DMembranePotentialMapping=ON -DNavierStokes=ON -Dcable_neuron=ON -DCNCpp11Features=OFF -Dneuro_collection=ON -DSmallStrainMechanics=OFF -DLuaShell=OFF .. &&
